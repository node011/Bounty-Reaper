...
| BPF integrity | `ebpf bpf_integrity --baseline --duration 300` | Verify BountyReper hook integrity, detect unauthorized BPF program loads |
...
**BPF integrity verification** takes a baseline of loaded BPF programs via `bpftool` and periodically verifies no BountyReper programs have been detached or tampered with. Also monitors `bpf()` syscall for unauthorized program loads.
...
```bash
# List all BountyReper eBPF programs on the system
ebpf cleanup

# Remove all BountyReper eBPF programs
