...
- [uv](https://docs.astral.sh/uv/) (runs the bundled MCP servers under `mcp/`)
...
`bootstrap.sh` runs `bun install`, creates an isolated venv for each bundled MCP
server, installs Chromium, and seeds `.env` from `.env.example`. It is
idempotent — re-run it after a pull that changes dependencies.

See [docs/SETUP.md](docs/SETUP.md) for what each MCP server needs, which
API keys are optional (all of them), and troubleshooting.
