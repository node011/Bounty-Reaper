...
import type { ClickTask, PageTask } from "../src/types.ts"
...
// Option tasks are always ClickTasks; narrow the PageTask union so role/label are visible.
function click(task: PageTask | undefined): ClickTask {
  if (task?.type !== "click") throw new Error(`expected a click task, got ${task?.type ?? "none"}`)
  return task
}
...
test("A: search-gated dialog combobox → options revealed by probing and collected", async () => {
  ...
  expect(click(tasks[0]).role).toBe("option")
  expect(click(tasks[0]).label).toContain("Owner")
...
test("B: inline listbox combobox → short-circuits to inline options, no probing", async () => {
  ...
  expect(click(tasks[0]).role).toBe("option")
  expect(click(tasks[0]).label).toBe("Active")
  await page.close()
})
