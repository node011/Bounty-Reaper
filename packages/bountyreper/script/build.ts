...
const modelsUrl = process.env.BOUNTYREPER_MODELS_URL || "https://models.dev"
...
const buildOSRaw = process.env.BOUNTYREPER_BUILD_OS?.trim()
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/bountyreper`,
      execArgv: [`--user-agent=bountyreper/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    entrypoints: ["./src/index.ts", parserWorker, workerPath],
    define: {
      BOUNTYREPER_VERSION: `'${Script.version}'`,
      BOUNTYREPER_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: `'${bunfsRoot + workerRelativePath}'`,
      BOUNTYREPER_WORKER_PATH: `'${bunfsRoot + workerPath.replace(/^\.\//, "").replace(/\.ts$/, ".js")}'`,
      BOUNTYREPER_CHANNEL: `'${Script.channel}'`,
      BOUNTYREPER_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
    },
  })
...
// ~/.local/share/bountyreper/node_modules/ and are installed by postinstall.
