...
function findBinary() {
  ...
  const packageName = `@bountyreper-io/bountyreper-${platform}-${arch}`
  const binaryName = platform === "windows" ? "bountyreper.exe" : "bountyreper"
...
function symlinkBinary(sourcePath, binaryName) {
  ...
  console.log(`bountyreper binary symlinked: ${targetPath} -> ${sourcePath}`)
...
function installWebUI() {
  ...
  const dataDir = path.join(xdgDataDir(), "bountyreper")
 ...
 * (~/.local/share/bountyreper/bin/hackbrowser-worker.js).
 *
 * The worker is the subprocess that runs playwright/hackbrowser so that
 * the main bountyreper binary has zero playwright references at startup
 * (subprocess.md). The main binary locates the worker at this path.
 *
 * Playwright JS package is also installed to
 * ~/.local/share/bountyreper/node_modules/ so the worker can resolve it
...
// packages/hackbrowser/package.json (and packages/bountyreper/package.json). The
  ...
  if (!fs.existsSync(playwrightDir)) {
    ...
    try {
      ...
      console.warn(
        ...
        err.message,
        `\nTo install manually: npm install --prefix ~/.local/share/bountyreper --save-exact playwright@${PLAYWRIGHT_VERSION}`,
      )
    }
  } else {
 ...
 * Corporate installs often skip PATH setup, leaving `bountyreper` unreachable
  ...
  try {
    ...
    console.warn(
      `Warning: Could not add npm global bin to PATH automatically.\n` +
        `If 'bountyreper' command is not found, add npm's global bin directory to your PATH manually:\n` +
...
async function main() {
  try {
    ...
    console.error("Failed to setup bountyreper binary:", error.message)
    process.exit(1)
  }
}
