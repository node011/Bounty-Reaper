...
test("returns default native agents when no config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(names).toContain("bountyreper")
...
test("bountyreper agent has correct default properties", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const cs = await Agent.get("bountyreper")
...
test("custom agent config overrides native agent properties", async () => {
    ...
    config: {
      agent: {
        bountyreper: {
          ...
          description: "Custom bountyreper agent",
          temperature: 0.7,
          color: "#FF0000",
        },
      },
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(cs?.description).toBe("Custom bountyreper agent")
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        bountyreper: { prompt: "Custom system prompt" },
      },
    },
  })
  await Instance.provide({
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreper", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const cs = await Agent.get("bountyreper")
        const skillDir = path.join(tmp.path, ".bountyreper", "skill", "perm-skill")
...
test("defaultAgent returns bountyreper when no default_agent config", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const agent = await Agent.defaultAgent()
      expect(agent).toBe("bountyreper")
    },
  })
})
...
test("defaultAgent returns next primary when bountyreper is disabled", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        bountyreper: { disable: true },
        fallback: {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await Agent.defaultAgent()
      // bountyreper is disabled, so it should return the next primary agent
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // bountyreper is the only primary agent, disabled = no primary visible agents remain
