...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const bountyreperModel = {
      ...openaiModel,
      providerID: "bountyreper",
      api: {
        id: "bountyreper-test",
    ...
    const result = ProviderTransform.message(msgs, bountyreperModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.bountyreper?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.bountyreper?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          bountyreper: { itemId: "msg_bountyreper" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              bountyreper: { itemId: "msg_bountyreper_part" },
    ...
    expect(result[0].providerOptions?.bountyreper?.itemId).toBe("msg_bountyreper")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.bountyreper?.itemId).toBe("msg_bountyreper_part")
