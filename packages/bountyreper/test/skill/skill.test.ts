...
test("discovers skills from .bountyreper/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreper", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreper", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".bountyreper", "skill", "dir-skill")
...
test("discovers multiple skills from .bountyreper/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".bountyreper", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".bountyreper", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreper", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.BOUNTYREPER_TEST_HOME
  process.env.BOUNTYREPER_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.BOUNTYREPER_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const bountyreperSkillDir = path.join(dir, ".bountyreper", "skill", "agent-skill")
      const bountyreperSkillsDir = path.join(dir, ".bountyreper", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(bountyreperSkillDir, "SKILL.md"),
        `---
name: bountyreper-skill
description: A skill in the .bountyreper/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(bountyreperSkillsDir, "SKILL.md"),
        `---
name: bountyreper-skill
description: A skill in the .bountyreper/skills directory.
---
