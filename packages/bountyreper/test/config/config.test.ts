...
const managedConfigDir = process.env.BOUNTYREPER_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "bountyreper.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "bountyreper.json") {
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "bountyreper.jsonc"),
        `{
        // This is a comment
        "$schema": "https://bountyreper.io/config.json",
        "model": "test/model",
  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(
          path.join(dir, "bountyreper.json"),
          JSON.stringify({
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "bountyreper.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "bountyreper.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .bountyreper directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreperDir = path.join(dir, ".bountyreper")
      await fs.mkdir(bountyreperDir, { recursive: true })
      const agentDir = path.join(bountyreperDir, "agent")
...
test("loads agents from .bountyreper/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreperDir = path.join(dir, ".bountyreper")
      await fs.mkdir(bountyreperDir, { recursive: true })

      const agentsDir = path.join(bountyreperDir, "agents")
...
test("loads commands from .bountyreper/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreperDir = path.join(dir, ".bountyreper")
      await fs.mkdir(bountyreperDir, { recursive: true })

      const commandDir = path.join(bountyreperDir, "command")
...
test("loads commands from .bountyreper/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreperDir = path.join(dir, ".bountyreper")
      await fs.mkdir(bountyreperDir, { recursive: true })

      const commandsDir = path.join(bountyreperDir, "commands")
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = JSON.parse(await Bun.file(path.join(tmp.path, "bountyreper.json")).text())
...
test("does not try to install dependencies in read-only BOUNTYREPER_CONFIG_DIR", async () => {
  ...
  const prev = process.env.BOUNTYREPER_CONFIG_DIR
  process.env.BOUNTYREPER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.BOUNTYREPER_CONFIG_DIR
    else process.env.BOUNTYREPER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable BOUNTYREPER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "bountyreper.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .bountyreper config
      const projectDir = path.join(dir, "project")
      const bountyreperDir = path.join(projectDir, ".bountyreper")
      await fs.mkdir(bountyreperDir, { recursive: true })
      ...
      // Local .bountyreper config with different plugins
      await Bun.write(
        path.join(bountyreperDir, "bountyreper.json"),
        JSON.stringify({
          $schema: "https://bountyreper.io/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .bountyreper config with some overlapping plugins
      await Bun.write(
        path.join(bountyreperDir, "bountyreper.json"),
        JSON.stringify({
          $schema: "https://bountyreper.io/config.json",
...
// Note: preload.ts sets BOUNTYREPER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .bountyreper config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .bountyreper directory config enables it
      const bountyreperDir = path.join(dir, ".bountyreper")
      await fs.mkdir(bountyreperDir, { recursive: true })
      await Bun.write(
        path.join(bountyreperDir, "bountyreper.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/bountyreper")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/bountyreper")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-bountyreper@2.4.3")).toBe("oh-my-bountyreper")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@bountyreper/plugin@2.0.0")).toBe("@bountyreper/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-bountyreper@2.4.3", "file:///project/.bountyreper/plugin/oh-my-bountyreper.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.bountyreper/plugin/oh-my-bountyreper.js")
  })
  ...
  test("local plugin directory overrides global bountyreper.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const bountyreperDir = path.join(projectDir, ".bountyreper")
        const pluginDir = path.join(bountyreperDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("BOUNTYREPER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"]
    process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .bountyreper/ directories when flag is set", async () => {
    const originalEnv = process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"]
    process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .bountyreper directory with a command
          const bountyreperDir = path.join(dir, ".bountyreper", "command")
          await fs.mkdir(bountyreperDir, { recursive: true })
          await Bun.write(path.join(bountyreperDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .bountyreper should NOT be in directories list
          const hasProjectBountyreper = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectBountyreper).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BOUNTYREPER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["BOUNTYREPER_CONFIG_DIR"]
      process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["BOUNTYREPER_CONFIG_DIR"]
      } else {
        process.env["BOUNTYREPER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("BOUNTYREPER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BOUNTYREPER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["BOUNTYREPER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["BOUNTYREPER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from BOUNTYREPER_CONFIG_DIR, not project
