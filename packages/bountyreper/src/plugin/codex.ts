import type { Hooks, PluginInput } from "@bountyreper-io/plugin"
...
<html>
  <head>
    <title>BountyReper - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to BountyReper.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>BountyReper - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput): Promise<Hooks> {
  return {
          ...
          authorize: async () => {
            const deviceResponse = await fetch(`${ISSUER}/api/accounts/deviceauth/usercode`, {
              ...
              headers: {
                ...
                "User-Agent": `bountyreper/${Installation.VERSION}`,
              },
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "bountyreper"
      output.headers["User-Agent"] =
        `bountyreper/${Installation.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`
