...
const TIPS = [
  ...
  "BountyReper runs {highlight}120+ OWASP test cases{/highlight} autonomously across web targets",
  "Use specialized agents for {highlight}recon{/highlight}, {highlight}exploitation{/highlight}, and {highlight}reporting{/highlight}",
  "BountyReper generates {highlight}HackerOne-ready{/highlight} vulnerability reports from verified findings",
  ...
  "BountyReper supports {highlight}15+ vulnerability skills{/highlight}: IDOR, SSRF, XSS, injection, misconfig, and more",
  ...
  // Local LLM & provider
  "Run {highlight}bountyreper provider add{/highlight} to connect local LLMs (vLLM, Ollama, llama.cpp)",
  ...
  // Config & agents
  "Add {highlight}.md{/highlight} files to {highlight}.bountyreper/agent/{/highlight} for specialized security personas",
  ...
  "Use {highlight}bountyreper run{/highlight} for non-interactive scripting and CI/CD integration",
  "Create {highlight}.ts{/highlight} files in {highlight}.bountyreper/tools/{/highlight} to define custom LLM tools",
