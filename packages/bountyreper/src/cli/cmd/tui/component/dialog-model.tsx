...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "bountyreper" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
        ...
        return [
          {
            ...
            category,
            disabled: provider.id === "bountyreper" && model.id.includes("-nano"),
            footer: model.cost?.input === 0 && provider.id === "bountyreper" ? "Free" : undefined,
            onSelect: () => {
              dialog.clear()
    ...
    const providerOptions = pipe(
      sync.data.provider,
      sortBy(
        (provider) => provider.id !== "bountyreper",
      ...
      flatMap((provider) =>
        pipe(
          ...
          map(([model, info]) => ({
            ...
            disabled: provider.id === "bountyreper" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "bountyreper" ? "Free" : undefined,
            onSelect() {
              dialog.clear()
