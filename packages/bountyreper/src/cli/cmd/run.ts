...
import { EOL } from "os"
import { createBountyreperClient, type Message, type BountyreperClient, type ToolPart } from "@bountyreper-io/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run bountyreper with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running bountyreper server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: BountyreperClient) {
    ...
    async function share(sdk: BountyreperClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.BOUNTYREPER_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: BountyreperClient) {
    ...
    if (args.attach) {
      const sdk = createBountyreperClient({ baseUrl: args.attach, directory })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      if (port !== undefined) {
        ...
        const sdk = createBountyreperClient({ baseUrl, directory: process.cwd() })
        return await execute(sdk)
      }
      ...
      const sdk = createBountyreperClient({ baseUrl: "http://bountyreper.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
