...
const AGENT_USERNAME = "bountyreper-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/bountyreper.yml"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/bountyreper-agent"
            const command =
...
jobs:
  bountyreper:
    if: |
      ...
      startsWith(github.event.comment.body, '/bountyreper')
    ...
    steps:
      ...
      - name: Run bountyreper
        uses: bounty-reper/BountyReper/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/bountyreper,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        ...
        const result = await SessionPrompt.prompt({
          ...
          // agent is omitted - server will use default_agent from config or fall back to "bountyreper"
          parts: [
            {
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("bountyreper-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `bountyreper/${type}-${hex}-${timestamp}`
        }
        return `bountyreper/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/bountyreper-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[bountyreper session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the bountyreper infrastructure after your response",
