...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall bountyreper and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall BountyReper")

    // Check for other running bountyreper instances
    const running = await getRunningInstances()
    if (running > 0) {
      prompts.log.warn(`${running} other bountyreper process${running > 1 ? "es" : ""} running`)
      if (!args.force) {
        ...
        if (!proceed || prompts.isCancel(proceed)) {
          prompts.outro("Stop other bountyreper processes first, then retry")
          return
        }
      }
    }
...
async function getRunningInstances(): Promise<number> {
  ...
  try {
    if (process.platform === "win32") {
      const result = await $`tasklist /FI "IMAGENAME eq bountyreper.exe" /FO CSV /NH`.quiet().nothrow()
      const lines = result.stdout
        .toString("utf8")
        .split("\n")
        .filter((l) => l.includes("bountyreper"))
      return Math.max(0, lines.length - 1)
    }
    const result = await $`pgrep -f bountyreper`.quiet().nothrow()
...
async function collectRemovalTargets(args: UninstallArgs, method: Installation.Method): Promise<RemovalTargets> {
  const directories: RemovalTargets["directories"] = [
    ...
    },
    {
      ...
      label: "Config",
      description: "bountyreper.json, provider settings",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @bountyreper-io/bountyreper",
      pnpm: "pnpm uninstall -g @bountyreper-io/bountyreper",
      bun: "bun remove -g @bountyreper-io/bountyreper",
      yarn: "yarn global remove @bountyreper-io/bountyreper",
      brew: "brew uninstall bountyreper",
      choco: "choco uninstall bountyreper",
      scoop: "scoop uninstall bountyreper",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (dataTarget && !dataTarget.keep) {
    const dbPath = path.join(dataTarget.path, "bountyreper.db")
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (err) {
      ...
      if (binDir.includes(".bountyreper")) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall bountyreper -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.info(
    `Per-project ${UI.Style.TEXT_NORMAL_BOLD}.bountyreper/${UI.Style.TEXT_NORMAL} directories (skill cache, project config) are not removed.`,
  )
  prompts.log.info(`  To find them: ${UI.Style.TEXT_DIM}find ~ -name ".bountyreper" -type d -maxdepth 5`)

  UI.empty()
  prompts.log.success("Thank you for using BountyReper!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# bountyreper") || content.includes(".bountyreper/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".bountyreper/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".bountyreper/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".bountyreper"))
    ) {
      continue
    }
