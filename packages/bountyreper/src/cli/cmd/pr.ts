...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run bountyreper",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for bountyreper session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found bountyreper session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`bountyreper import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting bountyreper...")
        UI.println()

        // Launch bountyreper TUI with session ID if available
        const { spawn } = await import("child_process")
        const bountyreperArgs = sessionId ? ["-s", sessionId] : []
        const bountyreperProcess = spawn("bountyreper", bountyreperArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          bountyreperProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`bountyreper exited with code ${code}`))
          })
          bountyreperProcess.on("error", reject)
        })
      },
    })
  },
