...
export async function cleanupAzure(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output: string[] = [
    dryRun
      ...
      : "[*] Cleaning up BountyReper artifacts from Azure...\n",
  ]
  let cleaned = 0

  output.push("[*] Checking for BountyReper automation runbooks (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReper role assignments...")
  ...
  if (assignments.exitCode === 0) {
    ...
    for (const r of roles) {
      ...
      if (!desc.includes("bountyreper") && !desc.includes("cs-")) continue
      if (dryRun) {
  ...
  output.push("\n[*] Checking for BountyReper app registrations (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReper snapshots (cs-snap-* prefix)...")
  const snapshots = await az(["snapshot", "list", "--query", "[?tags.bountyreper=='true']"], sub, timeout)
  ...
  output.push("\n[*] Checking for BountyReper policy exemptions (cs-exempt-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReper Event Grid subscriptions (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReper Event Hub consumer groups (cs-tap-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReper Lighthouse assignments...")
