...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@bountyreper-io/plugin"
import z from "zod"
...
export namespace ToolRegistry {
  ...
  async function all(): Promise<Tool.Info[]> {
    ...
    return [
      InvalidTool,
      ...(["app", "cli", "desktop"].includes(Flag.BOUNTYREPER_CLIENT) ? [QuestionTool] : []),
      BashTool,
      ReadTool,
      GlobTool,
      GrepTool,
      ...
      SkillTool,
      ApplyPatchTool,
      ...(Flag.BOUNTYREPER_EXPERIMENTAL_LSP_TOOL ? [LspTool] : []),
      ...
      tools
        .filter((t) => {
          ...
          if (t.id === "codesearch" || t.id === "websearch") {
            return model.providerID === "bountyreper" || Flag.BOUNTYREPER_ENABLE_EXA
          }
