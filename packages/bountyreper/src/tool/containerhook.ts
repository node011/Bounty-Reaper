...
const PROGRAMS = {
  ...
  cleanup_container: {
    description:
      "Remove all BountyReper-created containers, images, volumes, and networks (by label bountyreper=true). ALWAYS run before leaving",
    args: "[--dry-run]",
  },
...
async function imageBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  const create = await run(
    "docker",
    ["run", "-d", "--name", containerId, "--label", "bountyreper=true", base, "sh", "-c", "sleep 10"],
    timeout,
  )
  ...
  const commit = await run(
    "docker",
    ["commit", "--change", `ENTRYPOINT ["/entrypoint.sh"]`, "--change", `LABEL bountyreper=true`, containerId, name],
    timeout,
  )
...
async function cleanupContainer(_args: string[], timeout: number): Promise<HookResult> {
  ...
  const findings: Finding[] = []
  const output: string[] = ["[*] Cleaning up BountyReper container resources...\n"]

  const containers = await run("docker", ["ps", "-a", "--filter", "label=bountyreper=true", "-q"], timeout)
  if (containers.exitCode === 0) {
    const ids = containers.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReper containers: ${ids.length}`)
  ...
  const images = await run("docker", ["images", "--filter", "label=bountyreper=true", "-q"], timeout)
  if (images.exitCode === 0) {
    const ids = images.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReper images: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
  ...
  const volumes = await run("docker", ["volume", "ls", "--filter", "label=bountyreper=true", "-q"], timeout)
  if (volumes.exitCode === 0) {
    const ids = volumes.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReper volumes: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
  ...
  const networks = await run("docker", ["network", "ls", "--filter", "label=bountyreper=true", "-q"], timeout)
  if (networks.exitCode === 0) {
    const ids = networks.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReper networks: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
