...
function globalFiles() {
  ...
  if (Flag.BOUNTYREPER_CONFIG_DIR) {
    files.push(path.join(Flag.BOUNTYREPER_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.BOUNTYREPER_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.BOUNTYREPER_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no BOUNTYREPER_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.BOUNTYREPER_CONFIG_DIR, Flag.BOUNTYREPER_CONFIG_DIR).catch(() => [])
}
