...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for bountyreper.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal bountyreper sessions
...
bountyreper acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via bountyreper's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | bountyreper acp
```
...
### Mapping to BountyReper

ACP sessions map cleanly to bountyreper's internal session model:
