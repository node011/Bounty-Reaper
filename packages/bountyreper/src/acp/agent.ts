...
import type { AssistantMessage, Event, BountyreperClient, SessionMessageResponse } from "@bountyreper-io/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: BountyreperClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `bountyreper auth login` in the terminal",
        name: "Login with bountyreper",
        id: "bountyreper-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // BountyReper stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const bountyreperProvider = providers.find((p) => p.id === "bountyreper")
    if (bountyreperProvider) {
      if (bountyreperProvider.models["big-pickle"]) {
        return { providerID: "bountyreper", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(bountyreperProvider.models))
      if (best) {
        return {
