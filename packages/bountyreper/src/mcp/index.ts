...
export namespace MCP {
        ...
        try {
          ...
          if (error instanceof UnauthorizedError) {
            ...
            if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
              ...
              Bus.publish(TuiEvent.ToastShow, {
                title: "MCP Authentication Required",
                message: `Server "${key}" requires authentication. Run: bountyreper mcp auth ${key}`,
                variant: "warning",
                duration: 8000,
              }).catch((e) => log.debug("failed to show toast", { error: e }))
            }
  ...
  async function findConfigFiles() {
    ...
    for (const filename of ["bountyreper.jsonc", "bountyreper.json"]) {
      const found = await Filesystem.findUp(filename, Instance.directory, Instance.worktree)
      files.push(...found)
    }
    // .bountyreper directory config files
