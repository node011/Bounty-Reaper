...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/bountyreper"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "bountyreper")
      default:
        return "/etc/bountyreper"
    }
  }

  const managedConfigDir = process.env.BOUNTYREPER_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  export const state = Instance.state(async () => {
    ...
    // 1) Remote .well-known/bountyreper (org defaults)
    // 2) Global config (~/.config/bountyreper/bountyreper.json{,c})
    // 3) Custom config (BOUNTYREPER_CONFIG)
    // 4) Project config (bountyreper.json{,c})
    // 5) .bountyreper directories (.bountyreper/agents/, .bountyreper/commands/, .bountyreper/plugins/, .bountyreper/bountyreper.json{,c})
    // 6) Inline config (BOUNTYREPER_CONFIG_CONTENT)
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${key}/.well-known/bountyreper` })
        const response = await fetch(`${key}/.well-known/bountyreper`)
        if (!response.ok) {
        ...
        result = merge(result, await load(JSON.stringify(remoteConfig), `${key}/.well-known/bountyreper`))
    ...
    if (Flag.BOUNTYREPER_CONFIG) {
      result = merge(result, await loadFile(Flag.BOUNTYREPER_CONFIG))
      log.debug("loaded custom config", { path: Flag.BOUNTYREPER_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.BOUNTYREPER_DISABLE_PROJECT_CONFIG) {
      for (const file of ["bountyreper.jsonc", "bountyreper.json"]) {
    ...
    const directories = [
      Global.Path.config,
      // Only scan project .bountyreper/ directories when project discovery is enabled
      ...(!Flag.BOUNTYREPER_DISABLE_PROJECT_CONFIG
        ...
        : []),
      // Always scan ~/.bountyreper/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".bountyreper"],
    ...
    // .bountyreper directory config overrides (project and global) config sources.
    if (Flag.BOUNTYREPER_CONFIG_DIR) {
      directories.push(Flag.BOUNTYREPER_CONFIG_DIR)
      log.debug("loading config from BOUNTYREPER_CONFIG_DIR", { path: Flag.BOUNTYREPER_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(directories)) {
      if (dir.endsWith(".bountyreper") || dir === Flag.BOUNTYREPER_CONFIG_DIR) {
        for (const file of ["bountyreper.jsonc", "bountyreper.json"]) {
    ...
    if (Flag.BOUNTYREPER_CONFIG_CONTENT) {
      result = merge(result, JSON.parse(Flag.BOUNTYREPER_CONFIG_CONTENT))
      log.debug("loaded custom config from BOUNTYREPER_CONFIG_CONTENT")
    }
    ...
    if (Flag.BOUNTYREPER_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.BOUNTYREPER_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@bountyreper-io/plugin": targetVersion,
    }
    ...
    if (!(await PackageRegistry.available("@bountyreper-io/plugin", dir))) {
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@bountyreper-io/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@bountyreper-io/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@bountyreper-io/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const patterns = ["/.bountyreper/command/", "/.bountyreper/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const patterns = ["/.bountyreper/agent/", "/.bountyreper/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * getPluginName("oh-my-bountyreper@2.4.3") // "oh-my-bountyreper"
   ...
   * 2. Local bountyreper.json
   * 3. Global plugin/ directory
   * 4. Global bountyreper.json
   *
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-bountyreper", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-bountyreper@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: bountyreper.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for bountyreper serve and web commands"),
      command: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "bountyreper.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "bountyreper.jsonc"))),
    )
  ...
  export function globalConfigFile() {
    const candidates = ["bountyreper.jsonc", "bountyreper.json", "config.json"].map((file) =>
