#!/usr/bin/env node
// Hackbrowser worker — runs as a subprocess of the main bountyreper binary.
...
import { runCrawl } from "@bountyreper-io/hackbrowser/api"
import type { CrawlOptions, LogRecord, CSEvent } from "@bountyreper-io/hackbrowser/api"
...
function buildCrawlOptions(opts: WorkerOptions, signal: AbortSignal): CrawlOptions {
  ...
  return {
    ...
    panel: opts.panel,
    bountyreperUrl: opts.bountyreperUrl,
    model,
    cdp: opts.cdp,
    logSink,
    eventSink,
