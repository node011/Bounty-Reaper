...
function findBinary() {
  ...
  const packageName = `@bountyreaper-io/bountyreaper-${platform}-${arch}`
  const binaryName = platform === "windows" ? "bountyreaper.exe" : "bountyreaper"
...
function symlinkBinary(sourcePath, binaryName) {
  ...
  console.log(`bountyreaper binary symlinked: ${targetPath} -> ${sourcePath}`)
 ...
 * the main bountyreaper binary has zero playwright references at startup
 * (subprocess.md). The main binary locates the worker at this path.
 *
 * Playwright JS package is also installed to
 * ~/.local/share/bountyreaper/node_modules/ so the worker can resolve it
...
// packages/hackbrowser/package.json (and packages/bountyreaper/package.json). The
  ...
  if (!fs.existsSync(playwrightDir)) {
    ...
    try {
      ...
      console.warn(
        ...
        err.message,
        `\nTo install manually: npm install --prefix ~/.local/share/bountyreaper --save-exact playwright@${PLAYWRIGHT_VERSION}`,
      )
    }
  } else {
 ...
 * Corporate installs often skip PATH setup, leaving `bountyreaper` unreachable
  ...
  try {
    ...
    console.warn(
      `Warning: Could not add npm global bin to PATH automatically.\n` +
        `If 'bountyreaper' command is not found, add npm's global bin directory to your PATH manually:\n` +
