...
const modelsUrl = process.env.BOUNTYREAPER_MODELS_URL || "https://models.dev"
...
const buildOSRaw = process.env.BOUNTYREAPER_BUILD_OS?.trim()
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/bountyreaper`,
      execArgv: [`--user-agent=bountyreaper/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    entrypoints: ["./src/index.ts", parserWorker, workerPath],
    define: {
      BOUNTYREAPER_VERSION: `'${Script.version}'`,
      BOUNTYREAPER_MIGRATIONS: JSON.stringify(migrations),
      OTUI_TREE_SITTER_WORKER_PATH: `'${bunfsRoot + workerRelativePath}'`,
      BOUNTYREAPER_WORKER_PATH: `'${bunfsRoot + workerPath.replace(/^\.\//, "").replace(/\.ts$/, ".js")}'`,
      BOUNTYREAPER_CHANNEL: `'${Script.channel}'`,
      BOUNTYREAPER_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "",
    },
  })
...
// ~/.local/share/bountyreaper/node_modules/ and are installed by postinstall.
