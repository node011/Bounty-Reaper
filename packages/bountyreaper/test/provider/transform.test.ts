...
describe("ProviderTransform.providerOptions", () => {
  ...
  describe("anthropic thinking block binding", () => {
    ...
    sdks.forEach((sdk) => {
      test(`${sdk.npm} omits block_binding and the beta for unbound models`, async () => {
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const bountyreaperModel = {
      ...openaiModel,
      providerID: "bountyreaper",
      api: {
        id: "bountyreaper-test",
        url: "https://api.bountyreper.io",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, bountyreaperModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.bountyreaper?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.bountyreaper?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          bountyreaper: { itemId: "msg_bountyreaper" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              bountyreaper: { itemId: "msg_bountyreaper_part" },
    ...
    expect(result[0].providerOptions?.bountyreaper?.itemId).toBe("msg_bountyreaper")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.bountyreaper?.itemId).toBe("msg_bountyreaper_part")
