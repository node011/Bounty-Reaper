...
test("discovers skills from .bountyreaper/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreaper", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreaper", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".bountyreaper", "skill", "dir-skill")
...
test("discovers multiple skills from .bountyreaper/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".bountyreaper", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".bountyreaper", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".bountyreaper", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.BOUNTYREAPER_TEST_HOME
  process.env.BOUNTYREAPER_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.BOUNTYREAPER_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const bountyreaperSkillDir = path.join(dir, ".bountyreaper", "skill", "agent-skill")
      const bountyreaperSkillsDir = path.join(dir, ".bountyreaper", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(bountyreaperSkillDir, "SKILL.md"),
        `---
name: bountyreaper-skill
description: A skill in the .bountyreaper/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(bountyreaperSkillsDir, "SKILL.md"),
        `---
name: bountyreaper-skill
description: A skill in the .bountyreaper/skills directory.
---
