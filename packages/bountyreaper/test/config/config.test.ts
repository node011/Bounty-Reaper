...
const managedConfigDir = process.env.BOUNTYREAPER_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "bountyreaper.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "bountyreaper.json") {
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "bountyreaper.jsonc"),
        `{
        // This is a comment
        "$schema": "https://bountyreper.io/config.json",
        "model": "test/model",
  ...
  try {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(
          path.join(dir, "bountyreaper.json"),
          JSON.stringify({
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "bountyreaper.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "bountyreaper.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .bountyreaper directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreaperDir = path.join(dir, ".bountyreaper")
      await fs.mkdir(bountyreaperDir, { recursive: true })
      const agentDir = path.join(bountyreaperDir, "agent")
...
test("loads agents from .bountyreaper/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreaperDir = path.join(dir, ".bountyreaper")
      await fs.mkdir(bountyreaperDir, { recursive: true })

      const agentsDir = path.join(bountyreaperDir, "agents")
...
test("loads commands from .bountyreaper/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreaperDir = path.join(dir, ".bountyreaper")
      await fs.mkdir(bountyreaperDir, { recursive: true })

      const commandDir = path.join(bountyreaperDir, "command")
...
test("loads commands from .bountyreaper/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const bountyreaperDir = path.join(dir, ".bountyreaper")
      await fs.mkdir(bountyreaperDir, { recursive: true })

      const commandsDir = path.join(bountyreaperDir, "commands")
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = JSON.parse(await Bun.file(path.join(tmp.path, "bountyreaper.json")).text())
...
test("does not try to install dependencies in read-only BOUNTYREAPER_CONFIG_DIR", async () => {
  ...
  const prev = process.env.BOUNTYREAPER_CONFIG_DIR
  process.env.BOUNTYREAPER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.BOUNTYREAPER_CONFIG_DIR
    else process.env.BOUNTYREAPER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable BOUNTYREAPER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "bountyreaper.json"),
        JSON.stringify({ $schema: "https://bountyreper.io/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "bountyreaper.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .bountyreaper config
      const projectDir = path.join(dir, "project")
      const bountyreaperDir = path.join(projectDir, ".bountyreaper")
      await fs.mkdir(bountyreaperDir, { recursive: true })
      ...
      // Local .bountyreaper config with different plugins
      await Bun.write(
        path.join(bountyreaperDir, "bountyreaper.json"),
        JSON.stringify({
          $schema: "https://bountyreper.io/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .bountyreaper config with some overlapping plugins
      await Bun.write(
        path.join(bountyreaperDir, "bountyreaper.json"),
        JSON.stringify({
          $schema: "https://bountyreper.io/config.json",
...
// Note: preload.ts sets BOUNTYREAPER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .bountyreaper config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .bountyreaper directory config enables it
      const bountyreaperDir = path.join(dir, ".bountyreaper")
      await fs.mkdir(bountyreaperDir, { recursive: true })
      await Bun.write(
        path.join(bountyreaperDir, "bountyreaper.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/bountyreaper")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/bountyreaper")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-bountyreaper@2.4.3")).toBe("oh-my-bountyreaper")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@bountyreaper/plugin@2.0.0")).toBe("@bountyreaper/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-bountyreaper@2.4.3", "file:///project/.bountyreaper/plugin/oh-my-bountyreaper.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.bountyreaper/plugin/oh-my-bountyreaper.js")
  })
  ...
  test("local plugin directory overrides global bountyreaper.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const bountyreaperDir = path.join(projectDir, ".bountyreaper")
        const pluginDir = path.join(bountyreaperDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("BOUNTYREAPER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"]
    process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .bountyreaper/ directories when flag is set", async () => {
    const originalEnv = process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"]
    process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .bountyreaper directory with a command
          const bountyreaperDir = path.join(dir, ".bountyreaper", "command")
          await fs.mkdir(bountyreaperDir, { recursive: true })
          await Bun.write(path.join(bountyreaperDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .bountyreaper should NOT be in directories list
          const hasProjectBountyReaper = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectBountyReaper).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BOUNTYREAPER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["BOUNTYREAPER_CONFIG_DIR"]
      process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["BOUNTYREAPER_CONFIG_DIR"]
      } else {
        process.env["BOUNTYREAPER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("BOUNTYREAPER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BOUNTYREAPER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["BOUNTYREAPER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["BOUNTYREAPER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from BOUNTYREAPER_CONFIG_DIR, not project
