...
BountyReaper patterns. Critical for operational security — always run before
...
Part of BountyReaper offensive security platform.
...
# -- BountyReaper program name patterns --
# Programs matching any of these patterns will be flagged for removal
BOUNTYREAPER_PATTERNS = [
    r"^cs_",                  # cs_* prefix
    r"^bountyreaper_",         # bountyreaper_* prefix
...
COMPILED_PATTERNS = [re.compile(p, re.IGNORECASE) for p in BOUNTYREAPER_PATTERNS]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="BountyReaper eBPF Cleanup Tool - Enumerate and remove BountyReaper eBPF programs from the system",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  sudo python3 cleanup.py                      # List all eBPF programs
  sudo python3 cleanup.py --remove              # Remove BountyReaper programs
  ...
  sudo python3 cleanup.py --all                 # Show all programs (not just BountyReaper)
...
Operational Security:
  ...
  any eBPF tracing artifacts left by BountyReaper agents.
        """,
    )
    parser.add_argument(
        "--remove",
        action="store_true",
        default=False,
        help="Remove eBPF programs matching BountyReaper patterns",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Show all eBPF programs, not just BountyReaper ones",
    )
    parser.add_argument(
        "--json-output",
        action="store_true",
...
def is_bountyreaper_program(name):
    """Check if a program name matches any BountyReaper pattern."""
    if not name:
        return False
...
def enumerate_via_bpftool():
    ...
    for prog in progs:
        ...
        entry["is_bountyreaper"] = is_bountyreaper_program(entry["name"])
...
def parse_bpftool_text_output(text):
    ...
    for line in text.strip().split("\n"):
        ...
        if header_match:
            ...
            current["is_bountyreaper"] = is_bountyreaper_program(current["name"])
            continue
        ...
        for entry in bpf_path.rglob("*"):
            ...
            if entry.is_file() or entry.is_symlink():
                ...
                programs.append({
                    ...
                    "pids": [],
                    "source": "bpffs",
                    "is_bountyreaper": is_bountyreaper_program(name),
...
def print_text_output(programs, methods_used, show_all=False, removal_results=None):
    ...
    cs_programs = [p for p in programs if p["is_bountyreaper"]]
    other_programs = [p for p in programs if not p["is_bountyreaper"]]

    print("=" * 90)
    print(f"{'BountyReaper eBPF Cleanup Tool':^90}")
    print("=" * 90)
    print()
    ...
    print(f"  BountyReaper programs found: {len(cs_programs)}")
    print(f"  Other programs found: {len(other_programs)}")
    print()

    if cs_programs:
        print("-" * 90)
        print("  BOUNTYREAPER PROGRAMS (flagged for removal)")
        print("-" * 90)
    ...
    if not cs_programs and not removal_results:
        print("  [OK] No BountyReaper eBPF programs detected on this system.")
        print()
...
def main():
    ...
    if args.remove and cs_programs:
        if args.dry_run:
            if not args.json_output:
                print(f"[*] DRY RUN: Would remove {len(cs_programs)} BountyReaper program(s):")
                for p in cs_programs:
            ...
            if not args.force and not args.json_output:
                print(f"[!] About to remove {len(cs_programs)} BountyReaper eBPF program(s).")
    ...
    if args.json_output:
        output = {
            "tool": "bountyreaper-ebpf-cleanup",
            "methods_used": methods_used,
            "total_programs": len(programs),
            "bountyreaper_programs": len(cs_programs),
