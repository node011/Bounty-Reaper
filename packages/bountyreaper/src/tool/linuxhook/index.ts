...
import * as mod_credential from "./credential"
import * as mod_evasion from "./evasion"
import * as mod_exfil from "./exfil"
import * as mod_lateral from "./lateral"
import * as mod_network from "./network"
import * as mod_persistence from "./persistence"
import * as mod_privesc from "./privesc"
import * as mod_recon from "./recon"
...
const PROGRAMS = {
  ...
  cleanup_linux: {
    description:
      "Remove BountyReaper artifacts from Linux target — clear logs, remove persistence mechanisms, restore modified configs, clean temp files. ALWAYS run before leaving",
    args: "",
  },
  artifact_enum: {
    description:
...
// Classify a program by the category module its handler actually lives in, so
// the mapping cannot drift as programs are added. Handlers defined inline in
// `dispatch` fall back to name-based classification.
const CATEGORY_MODULES: Array<[string, Record<string, unknown>]> = [
