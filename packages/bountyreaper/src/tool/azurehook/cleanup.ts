...
export async function cleanupAzure(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output: string[] = [
    dryRun
      ...
      : "[*] Cleaning up BountyReaper artifacts from Azure...\n",
  ]
  let cleaned = 0

  output.push("[*] Checking for BountyReaper automation runbooks (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReaper role assignments...")
  ...
  if (assignments.exitCode === 0) {
    ...
    for (const r of roles) {
      ...
      if (!desc.includes("bountyreaper") && !desc.includes("cs-")) continue
      if (dryRun) {
  ...
  output.push("\n[*] Checking for BountyReaper app registrations (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReaper snapshots (cs-snap-* prefix)...")
  const snapshots = await az(["snapshot", "list", "--query", "[?tags.bountyreaper=='true']"], sub, timeout)
  ...
  output.push("\n[*] Checking for BountyReaper policy exemptions (cs-exempt-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReaper Event Grid subscriptions (cs-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReaper Event Hub consumer groups (cs-tap-* prefix)...")
  ...
  output.push("\n[*] Checking for BountyReaper Lighthouse assignments...")
