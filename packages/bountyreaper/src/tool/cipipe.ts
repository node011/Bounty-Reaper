...
async function cleanupCi(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (!repo) return { output: "[-] --repo OWNER/REPO required", findings: [] }
  const output: string[] = [
    dryRun ? "[*] CLEANUP DRY RUN — no changes\n" : "[*] Cleaning up BountyReaper CI/CD artifacts...\n",
  ]
  let cleaned = 0
          ...
          if (fileContent.exitCode === 0) {
            const del = await gh(
              [
                ...
                "DELETE",
                "-f",
                `message=Remove BountyReaper workflow`,
                "-f",
...
export const CipipeTool = Tool.define("cipipe", {
  ...
  async execute(params, ctx) {
    ...
    try {
      await Gate.ask(ctx, { tool: "cipipe", program: params.program, category: Gate.fromName(params.program) })
