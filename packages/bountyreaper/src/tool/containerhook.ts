...
const PROGRAMS = {
  ...
  cleanup_container: {
    description:
      "Remove all BountyReaper-created containers, images, volumes, and networks (by label bountyreaper=true). ALWAYS run before leaving",
    args: "[--dry-run]",
  },
...
async function imageBackdoor(args: string[], timeout: number): Promise<HookResult> {
  ...
  const create = await run(
    "docker",
    ["run", "-d", "--name", containerId, "--label", "bountyreaper=true", base, "sh", "-c", "sleep 10"],
    timeout,
  )
  ...
  const commit = await run(
    "docker",
    ["commit", "--change", `ENTRYPOINT ["/entrypoint.sh"]`, "--change", `LABEL bountyreaper=true`, containerId, name],
    timeout,
  )
...
async function cleanupContainer(_args: string[], timeout: number): Promise<HookResult> {
  ...
  const findings: Finding[] = []
  const output: string[] = ["[*] Cleaning up BountyReaper container resources...\n"]

  const containers = await run("docker", ["ps", "-a", "--filter", "label=bountyreaper=true", "-q"], timeout)
  if (containers.exitCode === 0) {
    const ids = containers.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReaper containers: ${ids.length}`)
  ...
  const images = await run("docker", ["images", "--filter", "label=bountyreaper=true", "-q"], timeout)
  if (images.exitCode === 0) {
    const ids = images.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReaper images: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
  ...
  const volumes = await run("docker", ["volume", "ls", "--filter", "label=bountyreaper=true", "-q"], timeout)
  if (volumes.exitCode === 0) {
    const ids = volumes.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReaper volumes: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
  ...
  const networks = await run("docker", ["network", "ls", "--filter", "label=bountyreaper=true", "-q"], timeout)
  if (networks.exitCode === 0) {
    const ids = networks.stdout.trim().split("\n").filter(Boolean)
    output.push(`[+] BountyReaper networks: ${ids.length}`)
    if (ids.length > 0 && !dryRun) {
...
export const ContainerhookTool = Tool.define("containerhook", {
  ...
  async execute(params, ctx) {
    ...
    try {
      await Gate.ask(ctx, { tool: "containerhook", program: params.program, category: Gate.fromName(params.program) })
