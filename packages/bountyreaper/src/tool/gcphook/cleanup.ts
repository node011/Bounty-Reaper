...
export async function cleanupGcp(args: string[], timeout: number): Promise<HookResult> {
  ...
  const output = [`[*] BountyReaper GCP cleanup — ${mode}`, `[*] Project: ${project}\n`]
  let cleaned = 0
  ...
  const snapR = await gcloud(
    [
      ...
      "snapshots",
      "list",
      "--filter=description~BountyReaper OR name~cs-",
      "--project",
      project,
      "--format=json",
    ],
  ...
  if (!dryRun) {
    ...
    findings.push({
      ...
      remediation: "Verify no BountyReaper artifacts remain",
    })
  }
