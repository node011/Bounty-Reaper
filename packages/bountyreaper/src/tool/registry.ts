...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@bountyreaper-io/plugin"
import z from "zod"
...
export namespace ToolRegistry {
  ...
  async function all(): Promise<Tool.Info[]> {
    ...
    return [
      InvalidTool,
      ...(["app", "cli", "desktop"].includes(Flag.BOUNTYREAPER_CLIENT) ? [QuestionTool] : []),
      BashTool,
      ReadTool,
      GlobTool,
      GrepTool,
      ...
      SkillTool,
      ApplyPatchTool,
      ...(Flag.BOUNTYREAPER_EXPERIMENTAL_LSP_TOOL ? [LspTool] : []),
      ...
      tools
        .filter((t) => {
          ...
          if (t.id === "codesearch" || t.id === "websearch") {
            return model.providerID === "bountyreaper" || Flag.BOUNTYREAPER_ENABLE_EXA
          }
