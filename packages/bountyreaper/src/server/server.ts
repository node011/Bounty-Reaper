    ...
    () =>
      ...
      app
        ...
        .use(
          cors({
            origin(input) {
              ...
              // *.bountyreper.io (https only)
              if (/^https:\/\/([a-z0-9-]+\.)*bountyreaper\.io$/.test(input)) {
                return input
              }
            ...
            credentials: true,
            allowHeaders: ["Authorization", "Content-Type", "x-bountyreaper-directory"],
          }),
        )
        .use(async (c, next) => {
          if (c.req.method === "OPTIONS") return next()
          const password = Flag.BOUNTYREAPER_SERVER_PASSWORD
          ...
          const username = Flag.BOUNTYREAPER_SERVER_USERNAME ?? "bountyreaper"
        ...
        .use(async (c, next) => {
          ...
          const explicit = c.req.query("directory") || c.req.header("x-bountyreaper-directory")
        ...
        .post(
          ...
          describeRoute({
            ...
            description: "Clean up and dispose the current BountyReaper instance, releasing all resources.",
        ...
        .get(
          ...
          describeRoute({
            ...
            description: "Get a list of all available commands in the BountyReaper system.",
        ...
        .get(
          ...
          describeRoute({
            summary: "List agents",
            description: "Get a list of all available AI agents in the BountyReaper system.",
        ...
        .all("/*", async (c) => {
          // In serve (API-only) mode, don't serve web UI — let app.bountyreper.io handle it
          if (!_serveWebUI)
            return c.json(
              { error: "API-only mode. Use app.bountyreper.io or 'bountyreaper web' for the web interface." },
              404,
            )
          ...
          if (_webDistDir === undefined) {
            const candidates = [
              // 1. ~/.bountyreaper/web/ (installed/production)
          ...
          // Remote fallback — proxy to app.bountyreper.io if deployed
          try {
            const response = await proxy(`https://app.bountyreper.io${reqPath}`, {
              ...c.req,
              headers: {
                ...c.req.raw.headers,
                host: "app.bountyreper.io",
              },
            })
            response.headers.set("Content-Security-Policy", csp)
            return response
            ...
            return c.json(
              { error: "Web UI not available. Run 'bun run build' in packages/app/ or deploy app.bountyreper.io" },
              503,
            )
          }
        }) as unknown as Hono,
