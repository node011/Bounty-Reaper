...
export namespace SessionPrompt {
  ...
  export const loop = fn(LoopInput, async (input) => {
    ...
    while (true) {
      ...
      if (disabledServers.length > 0) {
        system.push(
          [
            ...
            "If the user asks about capabilities these servers provide, inform them that the relevant MCP server is available but disabled, and they can enable it via the MCP panel or `bountyreaper mcp enable <name>`.",
          ].join("\n"),
        )
      }
      ...
      if (process.env["BOUNTYREAPER_PROMPT_DEBUG"]) {
  ...
  export async function resolveTools(input: {
    ...
    // ceiling). Auto-enabled for free models; force with BOUNTYREAPER_LITE_TOOLS=1/0.
    const envLite = process.env["BOUNTYREAPER_LITE_TOOLS"]
    const lite =
      envLite === "1" ||
      (envLite !== "0" &&
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    ...
    if (!Flag.BOUNTYREAPER_EXPERIMENTAL_PLAN_MODE) {
