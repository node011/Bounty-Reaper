...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.BOUNTYREAPER_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no BOUNTYREAPER_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.BOUNTYREAPER_CONFIG_DIR, Flag.BOUNTYREAPER_CONFIG_DIR).catch(() => [])
}
