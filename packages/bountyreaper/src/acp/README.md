...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for bountyreaper.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal bountyreaper sessions
...
bountyreaper acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via bountyreaper's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | bountyreaper acp
```
...
### Mapping to BountyReaper

ACP sessions map cleanly to bountyreaper's internal session model:
