...
import type { AssistantMessage, Event, BountyReaperClient, SessionMessageResponse } from "@bountyreaper-io/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: BountyReaperClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    private sdk: BountyReaperClient
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `bountyreaper auth login` in the terminal",
        name: "Login with bountyreaper",
        id: "bountyreaper-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // BountyReaper stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const bountyreaperProvider = providers.find((p) => p.id === "bountyreaper")
    if (bountyreaperProvider) {
      if (bountyreaperProvider.models["big-pickle"]) {
        return { providerID: "bountyreaper", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(bountyreaperProvider.models))
      if (best) {
        return {
