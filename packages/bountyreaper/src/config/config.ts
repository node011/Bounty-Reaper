...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/bountyreaper"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "bountyreaper")
      default:
        return "/etc/bountyreaper"
    }
  }

  const managedConfigDir = process.env.BOUNTYREAPER_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): https://bountyreper.io/docs/config#precedence-order
    // 1) Remote .well-known/bountyreaper (org defaults)
    // 2) Global config (~/.config/bountyreaper/bountyreaper.json{,c})
    // 3) Custom config (BOUNTYREAPER_CONFIG)
    // 4) Project config (bountyreaper.json{,c})
    // 5) .bountyreaper directories (.bountyreaper/agents/, .bountyreaper/commands/, .bountyreaper/plugins/, .bountyreaper/bountyreaper.json{,c})
    // 6) Inline config (BOUNTYREAPER_CONFIG_CONTENT)
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${key}/.well-known/bountyreaper` })
        const response = await fetch(`${key}/.well-known/bountyreaper`)
        if (!response.ok) {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://bountyreper.io/config.json"
        result = merge(result, await load(JSON.stringify(remoteConfig), `${key}/.well-known/bountyreaper`))
    ...
    if (Flag.BOUNTYREAPER_CONFIG) {
      result = merge(result, await loadFile(Flag.BOUNTYREAPER_CONFIG))
      log.debug("loaded custom config", { path: Flag.BOUNTYREAPER_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.BOUNTYREAPER_DISABLE_PROJECT_CONFIG) {
      for (const file of ["bountyreaper.jsonc", "bountyreaper.json"]) {
    ...
    const directories = [
      Global.Path.config,
      // Only scan project .bountyreaper/ directories when project discovery is enabled
      ...(!Flag.BOUNTYREAPER_DISABLE_PROJECT_CONFIG
        ? await Array.fromAsync(
            Filesystem.up({
              targets: [".bountyreaper"],
              start: Instance.directory,
              stop: Instance.worktree,
            }),
          )
        : []),
      // Always scan ~/.bountyreaper/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".bountyreaper"],
    ...
    // .bountyreaper directory config overrides (project and global) config sources.
    if (Flag.BOUNTYREAPER_CONFIG_DIR) {
      directories.push(Flag.BOUNTYREAPER_CONFIG_DIR)
      log.debug("loading config from BOUNTYREAPER_CONFIG_DIR", { path: Flag.BOUNTYREAPER_CONFIG_DIR })
    }

    const deps = []

    for (const dir of unique(directories)) {
      if (dir.endsWith(".bountyreaper") || dir === Flag.BOUNTYREAPER_CONFIG_DIR) {
        for (const file of ["bountyreaper.jsonc", "bountyreaper.json"]) {
    ...
    if (Flag.BOUNTYREAPER_CONFIG_CONTENT) {
      result = merge(result, JSON.parse(Flag.BOUNTYREAPER_CONFIG_CONTENT))
      log.debug("loaded custom config from BOUNTYREAPER_CONFIG_CONTENT")
    }
    ...
    const env = process.env.BOUNTYREAPER_PERMISSION
    if (env) {
      try {
        result.permission = mergeDeep(result.permission ?? {}, JSON.parse(env))
      } catch {
        log.warn("BOUNTYREAPER_PERMISSION contains invalid JSON, skipping")
      }
    }
    ...
    if (Flag.BOUNTYREAPER_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.BOUNTYREAPER_DISABLE_PRUNE) {
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@bountyreaper-io/plugin": targetVersion,
    }
    ...
    if (!(await PackageRegistry.available("@bountyreaper-io/plugin", dir))) {
      log.debug("plugin package unavailable in registry, skipping dependency install", { dir })
      return
    }
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@bountyreaper-io/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@bountyreaper-io/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@bountyreaper-io/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const patterns = ["/.bountyreaper/command/", "/.bountyreaper/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const patterns = ["/.bountyreaper/agent/", "/.bountyreaper/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * getPluginName("oh-my-bountyreaper@2.4.3") // "oh-my-bountyreaper"
   ...
   * 2. Local bountyreaper.json
   * 3. Global plugin/ directory
   * 4. Global bountyreaper.json
   *
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-bountyreaper", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-bountyreaper@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: bountyreaper.local)"),
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://bountyreper.io/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://bountyreper.io/docs/agents"),
      provider: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "bountyreaper.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "bountyreaper.jsonc"))),
    )
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://bountyreper.io/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://bountyreper.io/config.json"
  ...
  export async function update(config: Info) {
    const filepath = path.join(Instance.directory, "bountyreaper.json")
  ...
  export function globalConfigFile() {
    const candidates = ["bountyreaper.jsonc", "bountyreaper.json", "config.json"].map((file) =>
