...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    async anthropic(input) {
      ...
      // Subscription (Claude Pro/Max): bountyreaper's own OAuth credential,
    ...
    async bountyreaper(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["bountyreaper"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from bountyreaper.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://bountyreper.io/",
            "X-Title": "bountyreaper",
          },
        },
      }
    },
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `bountyreaper/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `bountyreaper auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "bountyreaper",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.BOUNTYREAPER_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
   ...
   * bountyreaper's Provider system.
   */
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("bountyreaper")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if bountyreaper or opencode provider is available before using it
    const defaultProv = await state().then(
      (state) => state.providers["bountyreaper"] ?? state.providers["opencode"] ?? state.providers["opencode-go"],
    )
    if (defaultProv) {
      const firstFreeOrSmall =
        Object.keys(defaultProv.models).find(
          (m) => m.includes("nano") || m.includes("mini") || m.includes("flash") || m.includes("free"),
        ) ?? Object.keys(defaultProv.models)[0]
      if (firstFreeOrSmall) {
        return getModel(defaultProv.id, firstFreeOrSmall)
      }
    }
  ...
  export function sort(models: Model[]) {
    return sortBy(
      ...
      [(model) => (model.cost?.input === 0 ? 0 : 1), "asc"],
  ...
  export async function defaultModel() {
    ...
    const available = Object.values(providers).filter((p) => Object.keys(p.models).length > 0)
    const provider =
      available.find((p) => configured.includes(p.id)) ??
      available.find((p) => p.id === "bountyreaper" || p.id === "opencode" || p.id === "opencode-go") ??
      available[0]
