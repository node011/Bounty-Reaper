...
export namespace ProviderTransform {
  export const OUTPUT_TOKEN_MAX = Flag.BOUNTYREAPER_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
  ...
  export function variants(model: Provider.Model): Record<string, Record<string, any>> {
    ...
    // Every other grok: xAI accepts reasoning_effort only on the grok-3-mini
    // family. grok-3 and grok-4 reject it, so they must not fall through to the
    // generic @ai-sdk/xai case below, which would offer low/medium/high.
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "bountyreaper" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      if (input.model.providerID.startsWith("bountyreaper")) {
        result["promptCacheKey"] = input.sessionID
  ...
  // messages above it, and rejects the request when any of that changes. bountyreaper
  ...
  function anthropicBlockBinding(model: Provider.Model, options: { [x: string]: any }) {
    ...
    // Consume the BountyReaper-only opt-out even on models outside the default scope.
