...
export namespace ProviderError {
  ...
  // fires on OpenAI models proxied through GitHub Copilot. BountyReaper cannot
  ...
  function error(providerID: string, error: APICallError) {
    if (providerID.includes("github-copilot") && error.statusCode === 403) {
      return "Please reauthenticate with the copilot provider to ensure your credentials work properly with BountyReaper."
    }
    ...
    if (isContentFilter(m)) {
      return {
        ...
        message:
          ...
          "(it also applies to OpenAI models proxied through GitHub Copilot) and cannot be disabled by BountyReaper. " +
