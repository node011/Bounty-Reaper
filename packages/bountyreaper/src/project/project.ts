  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Bun.file(path.join(dotgit, "bountyreaper"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.BOUNTYREAPER_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(dotgit, "bountyreaper"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.BOUNTYREAPER_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
