...
const AGENT_USERNAME = "bountyreaper-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/bountyreaper.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://bountyreper.io/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/bountyreaper-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.bountyreper.io/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  bountyreaper:
    if: |
      ...
      startsWith(github.event.comment.body, '/bountyreaper')
    ...
    steps:
      ...
      - name: Run bountyreaper
        uses: node011/Bounty-Reaper/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.bountyreper.io" : "https://bountyreper.io"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.bountyreper.io"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/bountyreaper,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        ...
        const result = await SessionPrompt.prompt({
          ...
          // agent is omitted - server will use default_agent from config or fall back to "bountyreaper"
          parts: [
            {
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("bountyreaper-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `bountyreaper/${type}-${hex}-${timestamp}`
        }
        return `bountyreaper/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/bountyreaper-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[bountyreaper session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the bountyreaper infrastructure after your response",
