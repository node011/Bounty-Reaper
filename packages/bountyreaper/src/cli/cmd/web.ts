...
export const WebCommand = cmd({
  ...
  describe: "start bountyreaper server and open web interface",
  handler: async (args) => {
    if (!Flag.BOUNTYREAPER_SERVER_PASSWORD) {
      UI.println(
        UI.Style.TEXT_DANGER_BOLD +
          "!  " +
          "BOUNTYREAPER_SERVER_PASSWORD is required. Set it with: export BOUNTYREAPER_SERVER_PASSWORD=yourpassword",
      )
      process.exit(1)
    }
