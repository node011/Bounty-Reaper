...
import { EOL } from "os"
import { createBountyReaperClient, type Message, type BountyReaperClient, type ToolPart } from "@bountyreaper-io/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run bountyreaper with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running bountyreaper server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: BountyReaperClient) {
    ...
    async function share(sdk: BountyReaperClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.BOUNTYREAPER_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: BountyReaperClient) {
    ...
    if (args.attach) {
      const sdk = createBountyReaperClient({ baseUrl: args.attach, directory })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      if (port !== undefined) {
        ...
        const sdk = createBountyReaperClient({ baseUrl, directory: process.cwd() })
        return await execute(sdk)
      }
      ...
      const sdk = createBountyReaperClient({ baseUrl: "http://bountyreaper.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
