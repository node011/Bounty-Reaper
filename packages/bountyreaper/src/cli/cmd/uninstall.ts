...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall bountyreaper and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall BountyReaper")

    // Check for other running bountyreaper instances
    const running = await getRunningInstances()
    if (running > 0) {
      prompts.log.warn(`${running} other bountyreaper process${running > 1 ? "es" : ""} running`)
      if (!args.force) {
        ...
        if (!proceed || prompts.isCancel(proceed)) {
          prompts.outro("Stop other bountyreaper processes first, then retry")
          return
        }
      }
    }
...
async function getRunningInstances(): Promise<number> {
  ...
  try {
    if (process.platform === "win32") {
      const result = await $`tasklist /FI "IMAGENAME eq bountyreaper.exe" /FO CSV /NH`.quiet().nothrow()
      const lines = result.stdout
        .toString("utf8")
        .split("\n")
        .filter((l) => l.includes("bountyreaper"))
      return Math.max(0, lines.length - 1)
    }
    const result = await $`pgrep -f bountyreaper`.quiet().nothrow()
...
async function collectRemovalTargets(args: UninstallArgs, method: Installation.Method): Promise<RemovalTargets> {
  const directories: RemovalTargets["directories"] = [
    ...
    },
    {
      ...
      label: "Config",
      description: "bountyreaper.json, provider settings",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @bountyreaper-io/bountyreaper",
      pnpm: "pnpm uninstall -g @bountyreaper-io/bountyreaper",
      bun: "bun remove -g @bountyreaper-io/bountyreaper",
      yarn: "yarn global remove @bountyreaper-io/bountyreaper",
      brew: "brew uninstall bountyreaper",
      choco: "choco uninstall bountyreaper",
      scoop: "scoop uninstall bountyreaper",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (dataTarget && !dataTarget.keep) {
    const dbPath = path.join(dataTarget.path, "bountyreaper.db")
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (err) {
      ...
      if (binDir.includes(".bountyreaper")) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall bountyreaper -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.info(
    `Per-project ${UI.Style.TEXT_NORMAL_BOLD}.bountyreaper/${UI.Style.TEXT_NORMAL} directories (skill cache, project config) are not removed.`,
  )
  prompts.log.info(`  To find them: ${UI.Style.TEXT_DIM}find ~ -name ".bountyreaper" -type d -maxdepth 5`)

  UI.empty()
  prompts.log.success("Thank you for using BountyReaper!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# bountyreaper") || content.includes(".bountyreaper/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".bountyreaper/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".bountyreaper/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".bountyreaper"))
    ) {
      continue
    }
