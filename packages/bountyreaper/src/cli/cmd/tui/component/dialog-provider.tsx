...
import type { ProviderAuthAuthorization } from "@bountyreaper-io/sdk/v2"
...
function ApiMethod(props: ApiMethodProps) {
  ...
  return (
    <DialogPrompt
      ...
      placeholder="API key"
      description={
        props.providerID === "bountyreaper" ? (
          <box gap={1}>
            <text fg={theme.textMuted}>
              BountyReaper Zen gives you access to all the best coding models at the cheapest prices with a single API
              key.
            </text>
            <text fg={theme.text}>
              Go to <span style={{ fg: theme.primary }}>https://bountyreper.io/zen</span> to get a key
            </text>
          </box>
        ) : undefined
      }
...
function LocalDoneStep(props: { providerID: string; modelIDs: string[] }) {
  ...
  return (
    <box paddingLeft={2} paddingRight={2} gap={1} paddingBottom={1}>
      ...
      <text fg={theme.textMuted}>{"Use: bountyreaper --model " + props.providerID + "/" + props.modelIDs[0]}</text>
    </box>
  )
}
