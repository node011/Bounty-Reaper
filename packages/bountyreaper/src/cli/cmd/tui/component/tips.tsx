...
const TIPS = [
  ...
  "BountyReaper runs {highlight}120+ OWASP test cases{/highlight} autonomously across web targets",
  "Use specialized agents for {highlight}recon{/highlight}, {highlight}exploitation{/highlight}, and {highlight}reporting{/highlight}",
  "BountyReaper generates {highlight}HackerOne-ready{/highlight} vulnerability reports from verified findings",
  ...
  "BountyReaper supports {highlight}15+ vulnerability skills{/highlight}: IDOR, SSRF, XSS, injection, misconfig, and more",
  ...
  "BountyReaper supports {highlight}HAR replay{/highlight} — replay captured traffic with modifications",
  ...
  // Local LLM & provider
  "Run {highlight}bountyreaper provider add{/highlight} to connect local LLMs (vLLM, Ollama, llama.cpp)",
  ...
  // Config & agents
  "Add {highlight}.md{/highlight} files to {highlight}.bountyreaper/agent/{/highlight} for specialized security personas",
  ...
  "Use {highlight}bountyreaper run{/highlight} for non-interactive scripting and CI/CD integration",
  "Create {highlight}.ts{/highlight} files in {highlight}.bountyreaper/tools/{/highlight} to define custom LLM tools",
