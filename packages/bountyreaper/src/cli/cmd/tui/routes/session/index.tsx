...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@bountyreaper-io/sdk/v2"
...
import { nextThinkingMode, reasoningSummary, useThinkingMode } from "@tui/context/thinking"
...
export function Session() {
  ...
  const showThinking = () => thinking.mode() === "show"
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  █▀▀▄ █▀▀   ${UI.Style.TEXT_DIM}bountyreaper -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        `  ▀▀▀  █ ▀▄  `,
      ].join("\n"),
    )
  })
        ...
        dialog.replace(() => (
          <DialogReport
            ...
            onGenerate={() => {
              prompt.set({
                input:
                  "Generate final report with generate_report tool. Fill all AI sections and save to .bountyreaper/reports/",
                parts: [],
              })
            }}
          />
...
function ReasoningPart(props: { last: boolean; part: ReasoningPart; message: AssistantMessage }) {
  ...
  const inMinimal = createMemo(() => !ctx.showThinking())
  const duration = createMemo(() => {
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.BOUNTYREAPER_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
