...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run bountyreaper",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for bountyreaper session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found bountyreaper session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`bountyreaper import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting bountyreaper...")
        UI.println()

        // Launch bountyreaper TUI with session ID if available
        const { spawn } = await import("child_process")
        const bountyreaperArgs = sessionId ? ["-s", sessionId] : []
        const bountyreaperProcess = spawn("bountyreaper", bountyreaperArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          bountyreaperProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`bountyreaper exited with code ${code}`))
          })
          bountyreaperProcess.on("error", reject)
        })
      },
    })
  },
